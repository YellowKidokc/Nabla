...
def adapt_method_run(packet: dict[str, Any], run: dict[str, Any]) -> dict[str, Any]:
    ...
    periodic = {f"marker_{i}_{name}": ([] if i in (3, 4) else "UNKNOWN") for i, name in enumerate((
        "scope", "home_domain", "native_domains", "bridged_domains", "object_type", "claim_family",
        "function_kind", "source_kind", "standing", "native_grade", "modality", "evidence_grade",
        "dispute", "publication_state", "component_state"), 1)}
